<? if ($currency): ?>	
    <div class="span2 box-grey marg-05em pad-04em">
        <p class="dark-grey"><?= sprintf('Курсы валют на %s', Date('d-m-Y H:i')) ?>
            <br/>&nbsp;
            <? foreach ($currency as $cur_name => $cur_value): ?>
                <br/><?= $cur_name ?>: <?= $cur_value ?>
            <? endforeach; ?>
            <? foreach ($benzin as $cur_name => $cur_value): ?>
        ...
        <input class="btn btn-success btn-small" type="submit" style="margin-top: -12px;" name="btn_refresh_cur" value="Обновить"/>
        <?= Form::close(); ?>
    </div>
<? endif; ?>
