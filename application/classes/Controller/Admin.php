...
class Controller_Admin extends Controller_Template {
    ...
    public function before() {
        ...
        $actions = array('index',
            ...
            'contacts', 'articles',
            'logout', 'login', 'search',
            'currency', 'stats', 'to_small', 'custom');
        $icons = array('icon-desktop',
            ...
            'icon2',
            'icon-search',
            'currency', 'custom', 'stats', 'to_small');
        $actions_rus = array('Главная',
            ...
            'Выход',
            'Поиск',
            'Валюта', 'Статистика посещений сайта', 'to_small', 'custom');
    ...
    public function action_stats() {
        $dt_load_currency = Model::factory('Config')->getValue('dt_load_currency');
        $benzin = Model::factory('Config')->getValue('benzin');
        if (!$dt_load_currency or !$benzin or ($this->request->post())):
            $currency = Сurrency::info_nacbank();
            $benzin = Сurrency::info_benzin();
            if ($currency):
                $dt_load_currency = time();
                Model::factory('Config')->setValue('dt_load_currency', $dt_load_currency);
                Model::factory('Config')->setValue('currency_data', $currency);

                Model::factory('Config')->setValue('dt_load_benzin', $benzin);
                Model::factory('Config')->setValue('benzin', $benzin);
            endif;
            ...
            $currency = Model::factory('Config')->getValue('currency_data');
            $benzin = Model::factory('Config')->getValue('benzin');
        endif;
    ...
    public function action_custom() {
        $view = View::factory('admin/custom/custom');
        $view->tree = ORM::factory('Custom')->get_tree();
        $this->template->content = $view;
    }
